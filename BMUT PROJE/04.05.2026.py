...
plt.show()
# =========================================================
# 9.5) ORDINAL (SIRALI) DEĞİŞKEN DÖNÜŞÜMÜ
# =========================================================

# Eğitim durumu sıralı olduğu için sayıya çeviriyoruz
egitim_map = {
    "İlkokul": 1,
    "Ortaokul": 2,
    "Lise": 3,
...
# Çalışma sürelerini sıralı hale getiriyoruz (örnek, kendi seçeneklerine göre düzenle)
calisma_map = {
    "0-5 saat": 1,
    "6-10 saat": 2,
    "11-15 saat": 3,
...
# 12) PREPROCESSOR
# =========================================================
from sklearn.feature_selection import RFE  # RFE ile önemli feature seçimi yapmak için
...
X_hazir = preprocessor.fit_transform(X)  # X verisini modele uygun sayısal hale getiriyoruz

if hasattr(X_hazir, "toarray"):          # sparse matrix geldiyse normal array'e çeviriyoruz
...
tum_feature_isimleri = sayisal_ozellikler + list(encoded_cat_features)  # tüm feature isimleri

X_hazir_df = pd.DataFrame(X_hazir, columns=tum_feature_isimleri)  # modelin gördüğü X tablosu

y_df = pd.DataFrame(y, columns=["Ortak_Basari_100"])  # hedef değişken tablosu

X_ornek_df = X_hazir_df.head(10).copy()  # hocaya göstermek için ilk 10 X
y_ornek_df = y_df.head(10).copy()        # hocaya göstermek için ilk 10 y
...
# =========================================================
# 12.2) RFE İLE ÖNEMLİ FEATURE SEÇİMİ
# =========================================================

rfe_rapor_model = LinearRegression()  # RFE feature önemini belirlemek için Linear Regression kullanıyor

rfe_rapor = RFE(
    estimator=rfe_rapor_model,
    n_features_to_select=15           # en önemli 15 feature seçiliyor, istersen 10/12/20 yapabilirsin
)

rfe_rapor.fit(X_hazir_df, y)           # RFE tüm hazırlanmış X üzerinden feature seçimi yapıyor
...
# =========================================================
# 12.2.1) MUTUAL INFORMATION İLE FEATURE ÖNEMİ
# =========================================================

# Mutual Information, her feature ile hedef değişken arasındaki ilişkiyi ölçer
# Skor ne kadar yüksekse, o feature hedef değişkenle o kadar ilişkili kabul edilir
mi_skorlari = mutual_info_regression(
    X_hazir_df,                 # modelin gördüğü sayısal X tablosu
    y,                          # hedef değişken
    random_state=42             # aynı sonuçları tekrar alabilmek için
)

# Skorları tablo haline getiriyoruz
mi_sonuclari_df = pd.DataFrame({
    "Feature": tum_feature_isimleri,  # feature isimleri
    "MI_Skoru": mi_skorlari           # mutual information skorları
})

# En yüksek skora sahip feature'lar üstte olacak şekilde sıralıyoruz
mi_sonuclari_df = mi_sonuclari_df.sort_values(
    by="MI_Skoru",
    ascending=False
...
# =========================================================
# 12.3) EXCEL DOSYASI OLARAK KAYDET VE DÜZENLE
# =========================================================
...
excel_duzenle("rfe_feature_secimi.xlsx")
excel_duzenle("mutual_information_skorlari.xlsx")
...
for model_name, regressor in models.items():
    pipeline = Pipeline(steps=[
        ("preprocessor", preprocessor),  # veriyi sayısal hale getirir
        ("feature_selection", RFE(
            estimator=LinearRegression(),  # RFE feature seçimini Linear Regression'a göre yapar
            n_features_to_select=15  # en önemli 15 feature seçilir
        )),
        ("regressor", regressor)  # seçilen feature'larla model eğitilir
    ])
